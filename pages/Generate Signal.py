...
if st.sidebar.button('Generate'):
    ...
    st.session_state['signal'][st.session_state['primaryKey']] = changeableSignal
...
# deleting the selected signals from the check boxes and from the st.session_state['signal']
...
ax1.plot(analogSignal_time, changeableSignal, color='red', linewidth=5)
ax1.grid()
...
if st.sidebar.button('add'):
    ...
    for checkBoxIndex in chosenCheckBoxes:
        ...
        summedSignal = summedSignal + st.session_state['signal'][checkBoxIndex]

    # if there is anything to plot
    if atLeastOneChecked:
        st.session_state['primaryKey'] = st.session_state['primaryKey'] + 1
        st.session_state['signal'][st.session_state['primaryKey']] = summedSignal

# expander for the generated signals check boxes
...
for index, sgnal in st.session_state['signal'].items():
    if st.session_state['checkBoxes'][index]:
        ...
        ax.plot(analogSignal_time, sgnal)
        ax.grid()
        st.plotly_chart(fig)
